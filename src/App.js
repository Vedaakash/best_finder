...
// CORRECTED: Added signOut and made sure the import from './firebase' is here
...
// --- All UI Components are now defined OUTSIDE of the main App component to fix the search bar issue ---
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-md relative animate-fade-in-up">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4">
                        <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor={`${type}-email`}>Email Address</label>
                        <input
                    ...
                    <div className="mb-6">
                        <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor={`${type}-password`}>Password</label>
                        <input
                            className="shadow appearance-none border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-green-500"
            ...
            <div className="bg-white p-4 rounded-full shadow-2xl max-w-2xl w-full mx-auto animate-zoom-in">
                <form onSubmit={handleSearch} className="flex items-center">
                    <div className="flex items-center text-gray-500 pl-4 pr-2">
                        <LocationIcon />
                        <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} className="w-48 ml-2 bg-transparent focus:outline-none text-sm" placeholder="Enter your location" />
                    </div>
                    <div className="h-8 border-l border-gray-200 mx-4"></div>
                    <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full text-lg bg-transparent focus:outline-none" placeholder="Search for 'Tomato' or 'Onion'..." />
...
const SearchResultsPage = ({ searchTerm, location, searchResults, setPage, setSearchTerm, setSearchResults }) => (
    <div className="min-h-screen bg-gray-50 pt-24 px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-7xl mx-auto">
         ...
         <button onClick={() => { setPage('home'); setSearchTerm(''); setSearchResults([]); }} className="mt-12 mx-auto block bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 shadow" >
            Back to Home
        </button>
    </div>
);
...
// This is now the "brain" of your application. It holds all the important state and logic.
export default function App() {
    ...
    // RESTORED: Your original mockResults array with all 6 items.
    const mockResults = [
    ...
    const renderPage = () => {
        switch (page) {
            ...
            case 'search':
                return <SearchResultsPage searchTerm={searchTerm} location={location} searchResults={searchResults} setPage={setPage} setSearchTerm={setSearchTerm} setSearchResults={setSearchResults} />;
            case 'about':
                return <AboutPage />;
            case 'contact':
                return <ContactPage />;
    ...
    return (
        <div className="bg-white font-sans">
            ...
            <Navbar 
                page={page} setPage={setPage} user={user} handleLogout={handleLogout} 
                setLoginModalOpen={setLoginModalOpen} setSignupModalOpen={setSignupModalOpen}
                setSearchTerm={setSearchTerm} setSearchResults={setSearchResults}
            />
            ...
            <AuthModal 
                type="login" isOpen={isLoginModalOpen} onClose={() => { setLoginModalOpen(false); setAuthError(null); }} 
                handleAuth={handleLogin} authError={authError} 
            />
            <AuthModal 
                type="signup" isOpen={isSignupModalOpen} onClose={() => { setSignupModalOpen(false); setAuthError(null); }} 
                handleAuth={handleSignup} authError={authError} 
            />
             <Footer/>
        </div>
    );
